...
public class Heap<T extends Comparable<T>>{
	...
	//Exercise 7:
	//find min, start at last leaf, and check backwards against other leafs 
	public T findMin(){
		int n = heapList.size()-1;
		int i = n;  //get the index of last item
		T min = heapList.get(i);
		
		i--;
		while( i*2+1>n && i*2+2>n && i>0){ //it's a leaf so check it
			T newMin = heapList.get(i);
